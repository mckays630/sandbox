...
updateTransferTable = function() {
	...
	$('<th>').html('Class').appendTo(tr);
	$.each(header, function(i,h) {
	    $('<th>').html(h).appendTo(tr);
	});
	tr.appendTo(tbody);

	$.each(data, function(rclass,count) {
	    if (rclass != 'release') {
		classArray[rclass] = data[rclass];
		tr = $('<tr>');
		th = $('<th>');
		l  = $('<label />').html(' '+rclass);
...
plotClass = function() {
    ...
    $.each(classArray, function(rclass,array) {
		...
		num = parseInt(classArray[rclass][i]);
		rows[r].push(num);
	    });
